...
class Plugin
{
    ...
    public static function wpAmeliaApiCall()
    {
        try {
            ...
            // Ajout de la route pour l'export ICS
            $app->get('/export/ics', function ($request, $response) use ($container) {
                require_once AMELIA_PATH . '/src/Infrastructure/API/Export/ICSExport.php';
                $icsExport = new \AmeliaBooking\Infrastructure\API\Export\ICSExport($container);
                return $icsExport->export($request, $response);
            });

            // Ajout de la route publique pour le calendrier ICS
            $app->get('/public/calendar.ics', function ($request, $response) use ($container) {
                require_once AMELIA_PATH . '/src/Infrastructure/API/Export/ICSExport.php';
                $icsExport = new \AmeliaBooking\Infrastructure\API\Export\ICSExport($container);
                return $icsExport->export($request, $response);
            });
